...
\abstract{%
The tour provides a useful vehicle for exploring high dimensional datasets. It works by combining a sequence of projections---the tour path---into an animation---the display method. Current display implementations in R are limited in their interactivity and portability, and give poor performance and jerky animations even for small datasets.
...
\pkg{detourr} is available on CRAN and GitHub, and can be installed using either of the following commands in R:
...
\item
  \texttt{fps}: frames per second with which to display the animation. Defaults to 30 but can be increased for a smoother animation or decreased for very large datasets.
\item
...
To address this, \pkg{detourr} uses TensorFlow.js (Abadi et al. (2016)) as the main library for storing data and projection matrices and performing matrix operations. TensorFlow.js requires the user to choose between one of three available backends:
...
\textbf{SVG} is commonly used for web-based visuals, including in software such as D3.js (Bostock, Ogievetsky, and Heer (2011)) with good support for interaction and animation. Kipp, Laa, and Cook (2019) use D3.js with SVG for rendering tours, but describe performance issues when the number of points gets close to 2,000. This is because while SVG is suitable for drawing large and complex shapes, performance can degrade when rendering many individual shapes.
...
One downside of using HTML5 Canvas elements is that custom logic is needed to determine where the mouse pointer is relative to visual elements when interactions occur. This issue is resolved by rendering the image twice; the first pass renders to the screen and the second renders to an invisible ``picking'' scene. The colours of the points in this picking scene correspond to the ID of the point that was rendered. When a mouse is hovered over a pixel or a set of pixels are selected, we simply check their colour in the picking scene to determine which point IDs relate to the event. Rendering the scene twice at each frame makes performance all the more important.
...
There are three display functions implemented in the \pkg{detourr} package: \texttt{show\_scatter()}, \texttt{show\_sage()}, and \texttt{show\_slice()}. All three support 2D and 3D tour paths, and are based on the core \texttt{show\_scatter()} function. In this section, we will delve into some of the implementation details of these functions and how the original and \emph{sage} display have been extended to three dimensions.
...
Despite the slice tour itself being equivalent to that in \pkg{tourr}, the implementation has been simplified. Laa, Cook, and Valencia (2020) calculate the distance as:
...
Laa, Cook, and Valencia (2020) provide a generalisation of equations \eqref{eq:nu} and \eqref{eq:xprime} for a projection plane passing through an arbitrary anchor point \(\mathbf{c}\) as follows:
...
Figure \ref{fig:slice-sphere} shows a slice tour implemented using equations \eqref{eq:xprime2} and \eqref{eq:nu3} with an anchor of \((1, 0, 0)\). Initially \(v_1\) is almost parallel to the projection plane, so the slice runs close to the origin and only the points near the outside of the hollow sphere are highlighted. As the tour progresses, \(v_1\) becomes nearly orthogonal to the projection plane, so the slice runs close to the edge of the sphere and only a small number of points near the centre of the visual are highlighted.
...
This differs from the original implementation described by Laa, Cook, and Lee (2021) in that we don't convert the Euclidean vectors to polar form, and instead apply the scaling directly to the Euclidean vectors. This removal of the conversion step was primarily to improve performance.

\hypertarget{extension-to-3d}{%
\subsubsection{Extension to 3D}\label{extension-to-3d}}

Laa, Cook, and Lee (2021) provide the equation for the relative projected volume at radius \(r\) on to a two dimensional disk for the 2-dimensional sage display. In this paper, we extend and implement the scatter, sage, and slice displays in 3D, and to do this we needed to calculate the relative projected volume for the case of a 3-dimensional projection.
...
The package website contains additional documentation with interactive examples and vignettes including a vignette on how to add a new display method to \pkg{detourr}:

\begin{itemize}
\tightlist
\item
  Package Website: \href{https://casperhart.github.io/detourr/}{https://casperhart.github.io/detourr}
\end{itemize}

The source code for \pkg{detourr} is hosted on GitHub at the following link:

\begin{itemize}
\tightlist
\item
  GitHub: \url{https://github.com/casperhart/detourr}
\end{itemize}
...
In the appendix of Ursula Laa and Valencia (2022) (equations 7--10) is a derivation for the relative projected volume of a ball of radius \(r\).
...
{\centering \includegraphics[width=0.7\linewidth]{detourr_files/figure-latex/radial-cdf-p-1} 
...
{\centering \includegraphics[width=0.7\linewidth]{detourr_files/figure-latex/radial-cdf-d-1} 
...
Cheng, Joe, Bhaskar Karambelkar, and Yihui Xie. 2022. \emph{{leaflet: Create Interactive Web Maps with the JavaScript 'Leaflet' Library}}. \url{https://CRAN.R-project.org/package=leaflet}.

\leavevmode\vadjust pre{\hypertarget{ref-crosstalk}{}}%
Cheng, Joe, and Carson Sievert. 2022. \emph{{crosstalk: Inter-Widget Interactivity for HTML Widgets}}. \url{https://rstudio.github.io/crosstalk/}.
...
Harrison, Paul. 2022. \emph{{langevitour: Langevin Tour}}. \url{https://CRAN.R-project.org/package=langevitour}.
...
Henry, Lionel, and Hadley Wickham. 2022. \emph{{tidyselect: Select from a Set of Strings}}. \url{https://CRAN.R-project.org/package=tidyselect}.
...
Ooms, Jeroen. 2021. \emph{{gifski: Highest Quality GIF Encoder}}. \url{https://CRAN.R-project.org/package=gifski}.
...
Ursula Laa, Andreas Buja, Dianne Cook, and German Valencia. 2022. {``Hole or Grain? A Section Pursuit Index for Finding Hidden Structure in Multiple Dimensions.''} \emph{Journal of Computational and Graphical Statistics} 31 (3): 739--52. \url{https://doi.org/10.1080/10618600.2022.2035230}.
...
Xie, Yihui, Joe Cheng, and Xianying Tan. 2022. \emph{{DT: A Wrapper of the JavaScript Library 'DataTables'}}. \url{https://CRAN.R-project.org/package=DT}.
